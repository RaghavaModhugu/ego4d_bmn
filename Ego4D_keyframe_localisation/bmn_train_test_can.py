...
import numpy as np
import pandas as pd
from datasets.CanonicalKeyframeLocalisation_bmn import CanonicalKeyframeLocalisation_v2
...
def train_BMN(data_loader, model, optimizer, epoch, bm_mask):
    ...
    for n_iter, data in tqdm.tqdm(enumerate(data_loader)):
        clip_uid, frames, labels, prec_labels, state_change_label, fps, label_match_score_start, label_match_score_end, label_confidence_score, keyframe_time, prec_frame_time, clip_length = data
        if keyframe_time.item() == -99:
            continue
...
def BMN_Train(cfg):
    ...
    if cfg.TRAIN.INIT_PRETRAIN_FEATURE == True:
        checkpoint = torch.load(cfg.TRAIN.CHECKPOINT + "/BMN_checkpoint_6.pth.tar", map_location = torch.device('cpu'))
        model.load_state_dict(checkpoint['state_dict'])
    model = model.to(device)
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=0.0001, #opt["training_lr"],
                           weight_decay=1e-4 #opt["weight_decay"]
                           )
    train_dataset = CanonicalKeyframeLocalisation_v2(cfg, 'train') #Ego4DKeyframeLocalisation(cfg, 'train')
    val_dataset = CanonicalKeyframeLocalisation_v2(cfg, 'val') #Ego4DKeyframeLocalisation(cfg, 'val')
    ...
    for epoch in tqdm.tqdm(range(7, cfg.TRAIN.EPOCH)):
...
def BMN_inference(cfg):
    ...
    checkpoint = torch.load(cfg.TRAIN.CHECKPOINT + "/BMN_checkpoint_6.pth.tar", map_location = device)
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()

    test_dataset = CanonicalKeyframeLocalisation_v2(cfg, 'train')
        ...
        for lll, data in tqdm.tqdm(enumerate(test_dataset_loader)):
            ...
            for idx  in range(tscale):
                ...
                if end_index<tscale:
                    ...
                    only_keyF.append([clip_uid[0], xmax, xmax_score, keyframe_time.item(), state_change_label.item(), clip_length.item()])
            # 遍历起始分界点与结束分界点的组合
            #new_props = []
            for idx in range(tscale):
                for jdx in range(tscale):
                    ...
                    if start_index < end_index and  end_index<tscale :
                        ...
                        #new_props.append([clip_uid[0], xmin, xmax, xmin_score, xmax_score, clr_score, reg_score, score, keyframe_time.item(), prec_frame_time.item(), state_change_label.item(), clip_length.item()])
        #new_props = np.stack(new_props)
        only_keyF = np.stack(only_keyF)
            #########################################################################

        #col_name = ["clip_uid","xmin", "xmax", "xmin_score", "xmax_score", "clr_score", "reg_score", "score", "Keyframe_time", "PreC_time", "State_change_label", "clip_length"]
        #new_df = pd.DataFrame(new_props, columns=col_name)
        #new_df.to_csv(os.path.join(cfg.DATA.OUTPUT_DIR, "{}.csv".format("test_results")), index=False)

        only_keyF_cols = ["clip_uid", "xmax", "xmax_score", "Keyframe_time", "State_change_label", "Clip_length"]
        new_df = pd.DataFrame(only_keyF, columns=only_keyF_cols)
        new_df.to_csv(os.path.join(cfg.DATA.OUTPUT_DIR, "{}.csv".format("train_results_only_keyF_e6")), index=False)
